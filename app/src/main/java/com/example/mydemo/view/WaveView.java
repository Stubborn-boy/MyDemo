...
public class WaveView extends View {
     ...
     * 波浪圆圈颜色
     */
    private int mColor = getResources().getColor(R.color.colorPrimary);
    /**
     * 第一个圆圈的半径(也就是圆形图片的半径)
     */
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < waveList.size(); i++) {
            ...
            Integer alpha = wave.alpha;
            mPaint.setAlpha(wave.alpha);
            // 绘制波浪圆
            Integer radius = wave.radius;
            if(isFill){
        ...
        if (waveList.get(waveList.size() - 1).radius == mWidth) {
           addWave();
        }
